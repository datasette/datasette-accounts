...
Ticket core-04: the link-start endpoint (password step-up + password-less
step-up-via-provider), the link/step-up intent handling in finish_login, and
the strand-guarded self + admin unlink.

Kept in its own file so tests/test_providers*.py (core 01/02/03) stay
...
The identities surfacing on the account + admin *page data* (identities,
has_password, linkable_providers, admin-list identities) lands with the
frontend in core-06; here the flows are asserted at the db + endpoint layer.
"""
