...
Ticket todos/auth-providers/07. Unlike the other provider tests — which register
a throwaway provider through pluggy — this file drives
...
The demo provider's ``handle`` runs the exact start → external redirect → IdP
...
async def _drive_login(ds, subject, *, username=None, name=None, session=None):
    ...
    renders its loud dev-only banner along the way.
    """
    cookies = {}
    if session:
    ...
    # 2. the pretend IdP page: loud dev-only banner + a form back to callback.
