...
def test_disable_provider_warns_with_count_and_keeps_admin_sessions(
    tmp_path, register_extern
):
    """The prompt says how many sessions the switch will end, and the CLI —
    having no acting web session to spare — keeps every admin account's
    sessions instead (an operator at the shell doesn't lock the admins out)."""
    import sqlite3
    ...
    ids = {
        r["username"]: r["id"]
        for r in query(db, "SELECT id, username FROM datasette_accounts_users")
    }
    conn = sqlite3.connect(db)
    for token, actor, provider in (
        ("admin-extern", ids["admin"], "extern"),
        ("alice-extern-1", ids["alice"], "extern"),
        ("alice-extern-2", ids["alice"], "extern"),
        ("alice-password", ids["alice"], "password"),
    ):
        conn.execute(
            "INSERT INTO datasette_accounts_sessions (token_sha256, actor_id, "
            "created_at, expires_at, last_seen_at, provider) VALUES "
            "(?, ?, '2026-01-01', '2999-01-01', '2026-01-01', ?)",
            [token, actor, provider],
        )
    conn.commit()
    conn.close()

    # Declining the prompt: the warning names the two non-admin extern
    # sessions, and nothing changes.
    declined = run("disable-provider", "extern", "-i", db, input="n\n")
    assert declined.exit_code != 0
    assert "signs out 2 sessions" in declined.output
    assert "admin accounts' sessions are kept" in declined.output
    assert len(query(db, "SELECT 1 FROM datasette_accounts_sessions")) == 4

    result = run("disable-provider", "extern", "--json", "-y", "-i", db)
    assert result.exit_code == 0
    assert json.loads(result.output)["sessions_revoked"] == 2
    left = query(db, "SELECT token_sha256 FROM datasette_accounts_sessions ORDER BY 1")
    assert [r["token_sha256"] for r in left] == ["admin-extern", "alice-password"]
...
def test_set_signups_validation_and_round_trip(tmp_path, register_extern):
    ...
    # 'auto' is refused for the password provider (its sign-ups always await
    # approval) — a clean error, no settings row.
    pw_auto = run("set-signups", "password", "auto", "-y", "-i", db)
    assert pw_auto.exit_code != 0
    assert "auto-activate" in pw_auto.output
    assert (
        query(
            db,
