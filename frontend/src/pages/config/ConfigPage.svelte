<script lang="ts">
  ...
  // The built-in password provider's sign-ups always await approval, so it
  // never offers auto-activate (the server refuses it too).
  function signupsOptions(p: Provider) {
    return p.builtin ? SIGNUPS.filter((s) => s.value !== "auto") : SIGNUPS;
  }
  function sessionsNoun(n: number) {
  ...
  async function toggleProvider(p: Provider) {
    ...
    if (
      prev &&
      !window.confirm(
        `Turn off ${p.label}? ${p.active_sessions} ${sessionsNoun(p.active_sessions)} signed in through it will be signed out (your own session stays).`,
      )
    ) {
      return;
    }
            ...
            <tr>
              ...
              <td>
                <div class="signups">
                  ...
                  >
                    {#each signupsOptions(p) as s (s.value)}
              ...
              <td class="muted">
                {#if !p.builtin}{p.linked_count} linked ·{/if}
                {p.active_sessions} signed in
              </td>
            </tr>
          {/each}
        </tbody>
