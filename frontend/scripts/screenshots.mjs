...
function buildShots(browser) {
  return {
    ...
    // "Continue with Demo PIN" button (seed.py enables the installed demo
    // provider).
    login: async () => {
      ...
      await page.getByText("Continue with Demo PIN").waitFor();
    ...
    config: async () => {
      ...
      await page
        ...
        .waitFor({ timeout: 15_000 });
      await page.getByRole("heading", { name: "Sign-in providers" }).waitFor();
      await page
    ...
    "account-sessions": async () => {
      ...
      await page.getByText("Demo PIN").first().waitFor();
      await freezeVolatile(page);
