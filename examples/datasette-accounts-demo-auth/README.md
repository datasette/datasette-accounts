...
Then the login page shows a **Continue with Demo PIN** button. Clicking
it lands on the demo's pretend-IdP page: pick any username (the demo's
`subject`), set a 4-digit PIN on first sign-in (that claims the subject), and
present the same PIN on every sign-in after.
...
class DemoProvider(AuthProvider):
    ...
    label = "Demo PIN"              # rendered as "Continue with {label}"
...
- **`@provider_gate(key)`** — wrap each route in this one-line decorator to get
  the route-level guarantees core asks of every provider: a disabled provider's whole URL surface
...
from datasette_accounts.providers import (
    ExternalIdentity, finish_login, read_state, start_state,
)

# start: get the state to send, redirect to the IdP (here: our pretend IdP page)
response = Response.redirect(idp_authorize_url)
state = start_state(datasette, request, response, provider="demo",
...
- **Wrap every route in `@provider_gate(key)`.** It gives each route the
  enabled-404 + CSRF-on-POST + method gate core asks of every provider route.
...
- **Always `read_state` on the callback.** It is the CSRF/replay defense for the
  ...
  signed state. Your start route calls `start_state`, which carries a
  core-minted link/step-up state through and otherwise mints a plain login
  one; `make_state` refuses to mint a link/step-up state that isn't bound to
  an actor, so there is no way to pick the intent up from the URL.
- **Treat the IdP's response body as untrusted input.** Wrap every `.json()`
  call and every key access in the same error handling as the HTTP call, so a
  malformed body is a "start over" 400 — never a 500, and never a subject
  built from a missing value (`str(None)` is the literal `"None"`, which core
  can't tell from a real id; `finish_login` does refuse an empty one).
- **OAuth callbacks must be `GET`s.** The state cookie is `SameSite=Lax`, so a
  cross-site `POST` (`response_mode=form_post`) arrives without it, and
  `provider_gate`'s CSRF gate expects JSON anyway. If an IdP only offers
  `form_post`, that callback needs its own ungated route that relies on
  `read_state` alone — and it still won't see the cookie; prefer the query
  response mode.
...
1. Login page → **Continue with Demo PIN** → `GET /-/demo-auth/start`.
2. `start` asks `start_state` for the state to send (a fresh login-intent one,
   or a core-minted link/step-up state carried through untouched) and
   redirects to `/-/demo-auth/idp` (the local "pretend IdP") carrying it.
