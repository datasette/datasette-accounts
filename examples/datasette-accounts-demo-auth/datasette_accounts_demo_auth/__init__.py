...
# The pretend IdP page: subject + PIN behind a loud development-only banner,
# submitting (via GET, so the whole loop is browser navigations exactly like a
# real OAuth authorize→callback round-trip) back to the provider's callback
# subpath. No templates, no static files. This is a stand-in for a real IdP's
# login screen — the seam a real OAuth redirect_uri would hit.
_IDP_FORM_HTML = """<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Demo sign-in — development only</title>
<style>
...
<body>
<div class="banner">
  ⚠ Development only — PINs are stored in plain text with no rate limiting.
  Never expose this provider on a real deployment.
</div>
<h1>Demo sign-in</h1>
<p class="help">
  Pick any username and a 4-digit PIN. Your first sign-in sets the PIN;
  use the same PIN every time after that.
</p>
{error}
...
async def start(datasette: Datasette, request: Request) -> Response:
    # start_state hands back the one state this route should send to the IdP:
    # a link / step-up flow arrives with a state already minted by
    # datasette-accounts (intent + actor_id ride in that signed cookie) and it
    # is carried through untouched; a fresh login gets a login-intent state
    # minted onto this response. Nothing about the flow's intent is ever read
    # from the query string. Then we redirect the visitor to our pretend IdP
    ...
    state = start_state(
        datasette, request, response, provider="demo", next=request.args.get("next")
    )
...
async def idp(datasette: Datasette, request: Request) -> Response:
    ...
    # to one of the _ERRORS keys — anything else renders no message. The state
    # value is the one query-supplied string that lands in the page, so it is
    # HTML-escaped into the hidden input (a genuine state is URL-safe and
...
async def callback(datasette: Datasette, request: Request) -> Response:
    ...
    return await finish_login(
        ...
        ExternalIdentity(
            provider="demo",
            subject=subject,
            # The IdP form submits only subject + pin; these optional query
            # params stand in for the profile data a real provider fetches
            # after its token exchange (absent, core derives from subject).
...
def register_routes():
    ...
    # provider 404s here — the route-level guarantees core asks of every
    # provider, the provider's own responsibility.
    return [
