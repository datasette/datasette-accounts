...
# The provider contract (what a sign-in provider package imports) plus the
# names blessed for provider test suites. Everything else in this module is a
# route-layer helper core's own routes use; it is not part of the contract.
__all__ = [
    "AuthProvider",
    "LocalIdentity",
    "ExternalIdentity",
...
# Every state carries one of these intents: a fresh sign-in, or one of the two
# core-minted account-linking legs (design §6).
INTENTS = ("login", "link", "step-up")
...
class AuthProvider:
    ...
    enabled bit (for external identities) and refuses a disabled
...
def provider_gate(key: str) -> Callable[[RouteHandler], RouteHandler]:
    ...
    Gives the route the three guarantees design §3 asks of every provider
    route:
    ...
    * **CSRF gate** on POST (``security.csrf_error`` → plain-text 403). The
      gate requires ``Content-Type: application/json``, so it suits a
      provider's own fetch()-driven POSTs, not an IdP posting a form to the
      callback (OAuth ``response_mode=form_post``) — such a callback must be a
      separate, ungated route that relies on ``read_state`` alone.
    * **405** on any method other than GET / HEAD / POST.

    Skipping this decorator cannot yield a session: ``finish_login`` re-checks
    the enabled bit before any external mint / provision / link, so an ungated
    route on a disabled provider still authenticates nobody. The gate is
...
class ExternalIdentity:
    """A proven external identity, handed to ``finish_login`` by a provider.

    ``finish_login`` maps ``(provider, subject)`` through the identities table,
    re-checks the provider's enabled bit, and applies the signups policy to an
    unmatched identity (design §4). Matching is by ``(provider, subject)`` ONLY,
    never email (decision D6). ``subject`` must be the IdP's non-empty, stable
    user id; ``finish_login`` refuses anything else so a provider that loses
    the id can never map every visitor onto one account.
    """
...
# The built-in username/password provider lives in providers/password.py; it
# imports this module for finish_login, and this module never imports it back.
...
class State(TypedDict):
    ...
    for the link / step-up flows (design §6), hence ``dict[str, str] | None``.
    """
...
def make_state(
    ...
    *,
    ...
    A provider's own start route mints only ``intent="login"`` states (use
    ``start_state``, which also carries a core-minted link / step-up state
    through). The ``link`` / ``step-up`` intents are minted by core alone and
    must be bound to an ``actor_id``; asking for one without is a programming
    error and raises rather than minting an unbound state.
    """
    if intent not in INTENTS:
        raise ValueError(f"Unknown state intent: {intent!r}")
    if intent != "login" and not actor_id:
        raise ValueError(f"A {intent!r} state must be bound to an actor_id")
...
def start_state(
    ...
    *,
    ...
    """The state value a provider's *start* route should send to its IdP.

    A link / step-up flow reaches a provider's start route with a state already
    minted by core (``?state=`` + the signed cookie, carrying the intent and the
    acting account): that one is returned as-is, never re-minted. Otherwise a
    fresh ``intent="login"`` state is minted onto ``response`` (its cookie) with
...
def provider_start_path(datasette: Datasette, key: str) -> str:
    ...
    target provider's route. Callers validate ``key`` against the registry
    first; an unknown key is a bug and raises (KeyError) rather than pointing
    the visitor at a path nothing serves."""
    return datasette.urls.path(get_registry(datasette)[key].start_path)


# --------------------------------------------------------------------------
# Session cookie + the one db.create_session call
# --------------------------------------------------------------------------
...
async def mint_session(
    ...
    The success half of a sign-in minus the periodic housekeeping, which
    finish_login runs around it. The one ``db.create_session`` call every
    sign-in reaches lives here. ``provider`` is stamped on the session row as
    provenance (which provider minted it — 'password' for the built-in flow,
    the external provider's key otherwise).
    """
    internal = datasette.get_internal_database()
...
async def finish_login(
    ...
    *,
    ...
    `ExternalIdentity` maps ``(provider, subject)`` through the identities table,
...
def _gate_reason(user: dict[str, Any] | None, *, external: bool) -> str | None:
    """Shared account gate for both identity kinds — disabled > expired >
    pending_approval precedence, matching authenticate()'s gates. Returns a
    login_audit reason string, or None when the account may sign in. External
    flows use the ``provider_*`` reasons so the admin audit distinguishes an SSO
    refusal from a password one."""
...
async def _finish_external(
    ...
    # runtime condition — fail loud, never silently map across providers.
    if identity.provider != provider_key:
        raise ValueError(
    ...
    # A provider that lost the IdP's id (a missing `sub`, an empty header) must
    # never map every such visitor onto one shared account: refuse, and record
    # why for the admin. Checked before the enabled re-check's audit row so the
    ...
    # call finish_login from anywhere. A disabled provider can never mint /
    # provision.
    ...
    # Linking intents (design §6) are handled BEFORE the login mint — for a
    # matched identity as much as an unmatched one. A link / step-up callback
    # must NEVER fall through to minting a session for the identity's owner: the
    # user is already signed in throughout linking, and treating a link callback
    # as a login would let an attacker who completes a link flow with a
    # *victim's* already-linked identity be signed in as that victim. The
    # found/not-found split is delegated so each intent decides explicitly.
    if intent == "step-up":
...
STRANDED_ACCOUNT_MESSAGE = "Set a password first — this is your only way to sign in."
...
async def _finish_step_up(
    ...
    What the proof unlocks is in the state's ``u``: a ``target`` to link (we 302
    into that provider's start with a fresh ``intent="link"`` state carrying the
    step-up proof, ``step_up={provider, at}``, honored ≤ TTL) or an ``unlink``
    ``{provider, subject}`` to remove from the account, performed right here.
    """
    internal = datasette.get_internal_database()
    ip = security.client_ip(datasette, request)
    actor_id = (state or {}).get("a")
    step_up = (state or {}).get("u") or {}
    target = step_up.get("target")
    unlink = step_up.get("unlink")
    # The identity must belong to the acting account (proof of control of an
    # existing method), the state must name exactly one thing to unlock, and —
    # like the link leg — the state's bound actor must be the LIVE session's
    ...
    if (
        ...
        or live["id"] != actor_id
        or existing is None
        or existing["user_id"] != actor_id
        or bool(target) == bool(unlink)
    ):
        await db.record_login_attempt(
    ...
    if unlink:
        ...
        if unlink.get("provider") == provider_key:
            await db.record_login_attempt(
                internal,
                None,
                ip,
...
async def _mint_and_respond(
    ...
    """Shared success tail for every finish_login path: build the response, mint
    the session (provenance = provider_key), run the periodic housekeeping,
