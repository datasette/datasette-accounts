...
verify (login) and the self-registration submit. The canonical routes in
``routes/api.py`` (`/-/login/api/authenticate`, `/-/register/api/submit`,
`/-/set-password/api/complete`) keep their paths and delegate here; every
successful sign-in terminates at ``finish_login``.

The timing-discipline tests monkeypatch ``averify_dummy`` / ``averify_password``
on this module to count KDF calls, so the verify half must call the names bound
here rather than reach the KDF another way.
"""
