...
from ..sessions import current_token_sha, list_own_sessions, token_sha256
...
async def config_page(datasette, request):
    ...
    own_token = current_token_sha(datasette, request)
    providers = [
        ProviderAdminRow(
            key=key,
            ...
            active_sessions=await db.count_sessions_for_provider(
                internal, key, keep_token_sha256=own_token
            ),
        )
        for key, provider in get_registry(datasette).items()
    ]
    page_data = ConfigPageData(**view, providers=providers).model_dump()
    return await _render(
