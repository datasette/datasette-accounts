...
async def link_identity(db, actor_id, user_id, identity):
    ...
    ``identity`` is a ``providers.ExternalIdentity``. The linking routes
    (core-04) call this; it is the single insert point for a new
    ``(provider, subject)`` row on an already-existing account.
    """
