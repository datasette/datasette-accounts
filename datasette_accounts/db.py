...
async def set_provider_signups(db, actor_id, key, mode):
    ...
    ``auto`` (provision an active account and sign in) is only meaningful for
    an external identity provider: the built-in password provider's signups
    always land in the approval queue, so ``auto`` is refused for it rather
    than accepted and silently treated as ``approval``.
    ...
    if mode not in SIGNUPS_MODES:
        raise ValueError(f"Invalid signups mode: {mode!r}")
    if key == "password" and mode == "auto":
        raise ValueError("Password sign-ups can't auto-activate")
...
def _slugify_username(hint):
    ...
    shapes the input; the caller decides whether it's usable. Dots survive, so
    ``alice.example.com`` slugifies to itself (a valid derived username).
    """
    if not hint:
        return ""
...
async def link_identity(db, actor_id, user_id, identity):
    ...
    ``identity`` is a ``providers.ExternalIdentity``. ``providers._finish_link``
    calls this; it is the single insert point for a new ``(provider, subject)``
...
async def unlink_identity(db, actor_id, target_user_id, provider, subject, *, admin):
    ...
    password and this is its only identity — the in-tx sibling of the last-admin
    guard (count + delete are atomic, no strand-by-race).
    """
    ...
    def write(conn):
        ...
        only_one = (
            gen.count_identities_for_user(conn, user_id=target_user_id) or 0
        ) <= 1
