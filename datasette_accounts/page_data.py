...
class ProviderAdminRow(BaseModel):
    ...
    linked_count: int
    # Live sessions this provider minted, other than the viewer's own — what
    # turning it off would sign out (the confirm names this number).
    active_sessions: int


class ConfigPageData(BaseModel):
    """The /-/admin/config page: site messages + sign-in providers (the password
    row's signups policy is the self-registration switch)."""
...
class UnlinkRequest(BaseModel):
    # The (provider, subject) to unlink from the signed-in account, plus the
    # same fresh proof of an existing method link-start takes: the account's
    # password, or (password-less) the key of ANOTHER linked provider whose
    # flow the user re-completes.
    provider: str
    subject: str
    password: Optional[str] = None
    step_up_provider: Optional[str] = None
